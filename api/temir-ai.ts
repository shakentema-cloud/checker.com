...
type TemirAssistantRequest = {
  ...
  currentPath: string;
  currentBoard: {
    board: Array<Array<{ color: "red" | "black"; type: "man" | "king" } | null>>;
...
function buildCurrentBoardSummary(currentBoard: TemirAssistantRequest["currentBoard"]): string {
...
function sanitizeNavigateAction(value: unknown) {
  if (!value || typeof value !== "object") return null;
  const candidate = value as Record<string, unknown>;
  if (!TEMIR_ROUTE_TARGETS.includes(candidate.path as (typeof TEMIR_ROUTE_TARGETS)[number])) return null;
  return {
    path: candidate.path,
...
function sanitizeAssistantResponse(payload: any, currentBoard: TemirAssistantRequest["currentBoard"]) {
  const navigate = sanitizeNavigateAction(payload?.navigate);
  const boardDemoId = TEMIR_BOARD_DEMO_IDS.includes(payload?.boardDemoId) ? payload.boardDemoId : null;
...
async function runTemirAi(body: TemirAssistantRequest) {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await runTemirAi(body);
